...
import { LayoutList, ChevronDown, RotateCcw, CheckSquare } from "lucide-react";
...
export function RaceChecklist() {
  ...
  return (
    <div className="absolute bottom-0 left-0 right-0 z-30">
      ...
      <div className="absolute inset-0 -top-[100dvh]" onClick={toggleChecklist} />

      <div
        className="relative rounded-t-2xl bg-card shadow-[0_-4px_32px_rgba(0,0,0,0.18)] backdrop-blur-md"
      ...
      >
        ...
        <div className="flex shrink-0 items-center justify-between px-4 pb-3">
          <div className="flex items-center gap-2">
            <LayoutList className="h-5 w-5 text-ocean" />
            <span className="text-base font-bold text-foreground">Pre-Race Checklist</span>
          </div>
          <button onClick={toggleChecklist} className="rounded-lg p-1.5 hover:bg-muted">
        ...
        <div className="shrink-0 border-t border-border px-4 py-2.5">
          <div className="mb-1.5 flex items-center justify-between text-xs">
            <span className="text-muted-foreground">{doneCount} / {totalItems} items</span>
            <span className={cn(
              "font-bold",
            ...
            return (
              <div key={cat.id}>
                <button
                  ...
                  className="flex w-full items-center justify-between px-4 py-3 hover:bg-muted/40 active:bg-muted/60"
                >
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className={cn("text-xs text-muted-foreground transition-transform",
                      isOpen ? "rotate-90" : ""
                    )}>▶</span>
                  </div>
                </button>
                      ...
                      return (
                        <button
                          ...
                          className={cn(
                            ...
                            isChecked ? "bg-green-500/8 hover:bg-green-500/12" : "hover:bg-muted/60"
                          )}
                        >
                          <div className={cn(
                            ...
                            {isChecked && (
                              <svg viewBox="0 0 10 10" className="h-full w-full fill-white p-0.5">
                                <path d="M1.5 5L4 7.5L8.5 2.5" stroke="white" strokeWidth="1.5" fill="none"
        ...
        <div className="shrink-0 flex items-center justify-between border-t border-border px-4 py-3">
          <button
            ...
            className="flex items-center gap-1.5 rounded-xl px-3 py-2 text-xs text-muted-foreground hover:bg-muted hover:text-foreground"
          >
